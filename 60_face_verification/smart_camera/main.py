...
from smart_camera.face_identification import FaceIdentification
import argparse 
import time
...
class Game(arcade.Window):
    ...
    def on_draw(self):
        ...
        if self.access == True:
            ...
            arcade.draw_text("Access denied",self.width//4,self.height//2,arcade.color.RED,28,16)
        elif self.access == "No camera": 
            arcade.draw_text("No camera detected",self.width//4,self.height//2,arcade.color.RED,28,16)
...
parser.add_argument('--npyfile', default="smart_camera/face_bank.npy", help='Npy face_bank file path')
...
if __name__ == "__main__":
    ...
    result = faceidentification.face_identification( opt.npyfile)
    game=Game(result)
    arcade.run()
