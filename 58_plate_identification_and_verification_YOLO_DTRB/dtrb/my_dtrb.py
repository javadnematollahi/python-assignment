...
class DTRB:
    def __init__(self, weights_path,opt) -> None:
        if opt.sensitive:
    ...
    def load_model(self, weights_path,opt):
        ...
        print('self.model input parameters', opt.imgH, opt.imgW, opt.num_fiducial, opt.input_channel, opt.output_channel,
              opt.hidden_size, opt.num_class, opt.batch_max_length, opt.Transformation, opt.FeatureExtraction,
              opt.SequenceModeling, opt.Prediction)
        self.model = torch.nn.DataParallel(self.model).to(self.device)

        # load self.model
        print('loading pretrained self.model from %s' % weights_path)
    ...
    def predict(self, image,opt, image_name=["numbers"]):
        ...
        with torch.no_grad():
            ...
            for img_name, pred, pred_max_prob in zip(image_name, preds_str, preds_max_prob):
                ...
                print(f'{img_name:25s}\t{pred:25s}\t{confidence_score:0.4f}',end="")
...
if __name__ == "__main__":
    plate_recognizer = DTRB("../weights/dtrb-recognizer/best_accuracy.pth")
    image = cv2.imread("../io/input/plate_result.jpg")
