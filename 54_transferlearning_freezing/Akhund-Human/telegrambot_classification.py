...
label_Akhund_Human=['از ما نیست','از ماست']
model = tf.keras.models.load_model('weights/flower_layerfreezing.h5')
model_Akhund_Human = tf.keras.models.load_model('weights/Akhund_Human.h5')
...
def photo_human(message):
	global image_human,model_Akhund_Human,label_Akhund_Human,image,model,labels
	if image_human == 1 : 
			file_info = bot.get_file(message.photo[len(message.photo)-1].file_id)
			downloaded_file = bot.download_file(file_info.file_path)
			with open("AH.jpg", "wb") as new_file:
				new_file.write(downloaded_file)

			# user_image = cv2.imread('AH.jpg')
			all_face_landmarks = landmarks_detector.get_landmarks('AH.jpg')
			for i, face_landmarks in enumerate(all_face_landmarks):              
				user_image = align_face('AH.jpg', face_landmarks)
			...
			result = model_Akhund_Human.predict(user_image)
			out = np.argmax(result)
			# print(result)
			print(out)
			bot.send_message(message.chat.id , f'{label_Akhund_Human[out]}')
			image_human = 0 
	if image == 1 : 
		file_info = bot.get_file(message.photo[len(message.photo)-1].file_id)
		downloaded_file = bot.download_file(file_info.file_path)
...
def echo_all(message):
	...
	if image_human == 1 : 
		bot.send_message(message.chat.id, " عکس یه انسان گفتم بفرست بیزحمت ")	
		image_human=0       
