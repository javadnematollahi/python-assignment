...
class EatFood():
    ...
    def eat_pear(self):
        ...
        self.data.food_right = 0 
        self.data.food_left = 0 
        self.data.food_up = 0 
        self.data.food_down = 0 
        ...
        self.data.list_input.append({
            "last_up":self.data.last_up, "last_down":self.data.last_down, "last_right":self.data.last_right,
            "last_left":self.data.last_left, "food_up":self.data.food_up, "food_down":self.data.food_down,
            "food_left":self.data.food_left, "food_right":self.data.food_right, "dist_x_to_body":self.data.dist_x_to_body,
            "dist_y_to_body":self.data.dist_y_to_body, "dist_to_top_wall":self.data.dist_to_top_wall,
            "dist_to_down_wall":self.data.dist_to_down_wall, "dist_to_left_wall":self.data.dist_to_left_wall,
            "dist_to_right_wall":self.data.dist_to_right_wall, "Pear_center_x":self.game_class.pear.center_x,
            "Pear_center_y":self.game_class.pear.center_y, "Pear_to_top":800-self.game_class.pear.center_y,
            "Pear_to_bottom":self.game_class.pear.center_y, "Pear_to_right":800-self.game_class.pear.center_x,
            "Pear_to_left":self.game_class.pear.center_x
            })
        self.data.list_output.append({"Direction":self.data.direction})
        if self.game_class.snake.change_x != 0:
            if self.save == 0:
                if self.game_class.pear.center_x-16 <= self.game_class.snake.center_x and self.game_class.snake.change_x == 1 or\
                   self.game_class.snake.center_x <= self.game_class.pear.center_x+16 and self.game_class.snake.change_x == -1:
                    self.direction = 2
            for i,part in enumerate(self.game_class.snake.body):
                if self.game_class.snake.center_y == part['y'] and part['x']-128 < self.game_class.snake.center_x < part['x'] and\
                   self.game_class.snake.change_x == 1:
                    self.direction = 2
                    self.immediate_change_right = 1
                    print('immediate_change_right')
                    self.save = 1
                if self.game_class.snake.center_y == part['y'] and (part['x']) < self.game_class.snake.center_x < (part['x']+128) and\
                   self.game_class.snake.change_x == -1:
                    self.direction = 2
        ...
        if self.game_class.snake.change_y!=0:
            if self.save == 0:
                if self.game_class.pear.center_y-16 <= self.game_class.snake.center_y and self.game_class.snake.change_y == 1 or\
                   self.game_class.snake.center_y <= self.game_class.pear.center_y+16 and self.game_class.snake.change_y == -1:
                    self.direction = 0
            for i,part in enumerate(self.game_class.snake.body):
                if self.game_class.snake.center_x == part['x'] and part['y']-128 < self.game_class.snake.center_y < part['y'] and\
                   self.game_class.snake.change_y == 1:
                    self.direction = 0
                    self.immediate_change_up = 1
                    print('immediate_change_up')
                    self.save = 1
                if self.game_class.snake.center_x == part['x'] and (part['y']) < self.game_class.snake.center_y < (part['y']+128) and\
                   self.game_class.snake.change_y == -1:
                    self.direction = 0
...
class Game(arcade.Window):
    ...
    def on_key_release(self, symbol, modifiers):
        if symbol == arcade.key.UP:
            ...
            df_input.to_csv('dataset/input_data.csv', mode='a', index=False)
            df_output = pd.DataFrame(self.eat.data.list_output)
            df_output.to_csv('dataset/output_data.csv', mode='a', index=False)
            arcade.close_window()
            arcade.exit()
