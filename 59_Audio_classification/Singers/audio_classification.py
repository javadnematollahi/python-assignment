...
class Audio_classification:
    ...
    def make_model(self, activation='relu',optimizer=tf.keras.optimizers.Adam(), loss='categorical_crossentropy'):
        self.model = tf.keras.models.Sequential([
            tf.keras.layers.Conv1D(32, kernel_size=40, strides=16, activation=activation,input_shape=(48000,1)),
            tf.keras.layers.MaxPool1D(4),
            tf.keras.layers.Conv1D(32, kernel_size=3, activation=activation),
            tf.keras.layers.MaxPool1D(4),
            tf.keras.layers.Conv1D(32, kernel_size=3, activation=activation), 
            tf.keras.layers.MaxPool1D(4),  
            tf.keras.layers.Conv1D(32, kernel_size=3, activation=activation), 
    ...
    def fit_model(self, best_model_path="best_singer_model", monitor_for_bset_model='val_accuracy',monitor_for_early_stop='val_accuracy',epochs=50):
    ...
    def inference_model(self, sample, singer_list='singer_voices/dataset', model_path="best_singer_model"):
        if isinstance(singer_list, str):
            singers = []
            for f in os.listdir(singer_list):
                singers.append(f)
            print(singers)
        elif isinstance(singer_list, list):
            singers = singer_list
        audio  = pydub.AudioSegment.from_file(sample)
        audio = audio.set_sample_width(2)
        audio = audio.set_frame_rate(48000)
        audio = audio.set_channels(1)
        chunks = pydub.silence.split_on_silence(audio, min_silence_len=1000, silence_thresh=-25)
        result = sum(chunks)
        chunkses = pydub.utils.make_chunks(result, 1000)
        os.makedirs('singer_sample', exist_ok=True)
        for fr in os.listdir('singer_sample'):
            os.remove(os.path.join('singer_sample',fr))

        for i,chunk in enumerate(chunkses):
            if len(chunk) >= 1000:
                chunk.export(os.path.join('singer_sample', f'voice_{i}.wav'), format='wav')
        loaded_model = tf.keras.models.load_model(model_path)

        preds = []
        for f in os.listdir('singer_sample'):
            if os.path.isfile(os.path.join('singer_sample',f)):
                x = tf.io.read_file(os.path.join('singer_sample',f))
        ...
        unique=list(unique)
        num=list(num)
        print(singers[unique[np.argmax(num)]])

if __name__ == "__main__":
    audio_classify = Audio_classification()
    audio_classify.merge_voices("singer_voices/singer", "singer_voices/stick_data")
    audio_classify.remove_silence_and_to_wav('singer_voices/stick_data', 'singer_voices/rm_silence')
    audio_classify.split_voices('singer_voices/rm_silence', 'singer_voices/dataset')
    audio_classify.make_train_validation("singer_voices/dataset")
    audio_classify.make_model()
    audio_classify.fit_model()
    audio_classify.evaluate_model("best_singer_model")
    audio_classify.inference_model("singer_voices/test/chavoshi_test.wav")
